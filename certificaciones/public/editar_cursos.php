...
function diasToArray($dias_str)
{
    ...
    return array_filter(array_map(function ($dia) {
        ...
        <?php else: ?>
            <?php foreach ($cursos as $curso): ?>
                ...
                if ($curso['promotor']) {
                    ...
                    if ($p_data)
                        $nombre_promotor_actual = $p_data['nombre'] . ' ' . $p_data['apellido'];
                }
                ?>
                <div class="accordion-item">
                    <h2 class="accordion-header" id="heading<?= $curso['id_curso'] ?>">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                            data-bs-target="#collapse<?= $curso['id_curso'] ?>" aria-expanded="false"
                            aria-controls="collapse<?= $curso['id_curso'] ?>">
                            ...
                            <?php if ($curso['estado']): ?>
                                <span class="badge bg-primary ms-2"><i class="fas fa-play"></i> Activo</span>
                            <?php else: ?>
                                <span class="badge bg-secondary ms-2"><i class="fas fa-flag-checkered"></i> Finalizado</span>
                            <?php endif; ?>

                            <span class="badge bg-secondary ms-2">ID: <?= $curso['id_curso'] ?></span>
                        </button>
                    </h2>
                    <div id="collapse<?= $curso['id_curso'] ?>" class="accordion-collapse collapse"
                        aria-labelledby="heading<?= $curso['id_curso'] ?>" data-bs-parent="#accordionCursos">
                        <div class="accordion-body">
                            <form id="editarCursoForm<?= $curso['id_curso'] ?>" action="../controllers/curso_controlador.php"
                                method="post" class="editarCursoForm">
                                <input type="hidden" name="action" value="editar">
                                <input type="hidden" name="id_curso" value="<?= $curso['id_curso'] ?>">
                                <input type="hidden" name="modulos_a_eliminar" id="modulos_a_eliminar_<?= $curso['id_curso'] ?>"
                                    value="">
                                    ...
                                    <div class="card-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="nombre_curso"
                                                value="<?= h($curso['nombre_curso']) ?>" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Descripción</label>
                                            <textarea class="form-control" name="descripcion"
                                                required><?= h($curso['descripcion']) ?></textarea>
                                        </div>
                                        <div class="row">
                                                ...
                                                <div class="p-2 border rounded bg-light">
                                                    ...
                                                    foreach ($dias_semana as $dia):
                                                        ...
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input" type="checkbox" name="dias_clase[]"
                                                                value="<?= $dia ?>" id="dia_<?= $curso['id_curso'] ?>_<?= $dia ?>"
                                                                <?= $checked ?>>
                                                            <label class="form-check-label"
                                                                for="dia_<?= $curso['id_curso'] ?>_<?= $dia ?>"><?= ucfirst($dia) ?></label>
                                                        </div>
                                                    <?php endforeach; ?>
                                                </div>
                                            </div>
                                ...
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3">
                                        <h6 class="m-0 font-weight-bold text-primary">Gestión y Autorización</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <div class="input-group">
                                                    <input type="hidden" id="promotor_id_<?= $curso['id_curso'] ?>"
                                                        name="promotor" value="<?= h($curso['promotor']) ?>" required>
                                                    <input type="text" class="form-control bg-white"
                                                        id="promotor_nombre_<?= $curso['id_curso'] ?>"
                                                        value="<?= h($nombre_promotor_actual) ?>" readonly>
                                                    <button class="btn btn-outline-primary" type="button"
                                                        onclick="abrirModalFacilitador(<?= $curso['id_curso'] ?>)">
                                                ...
                                                <div class="card border-left-warning h-100 py-2">
                                                    <div class="card-body">
                                                        <div class="row no-gutters align-items-center">
                                                            <div class="col mr-2">
                                                                ...
                                                                <?php if ($curso['autorizacion']):
                                                                    ...
                                                                    ?>
                                                                    <div
                                                                        class="h5 mb-0 font-weight-bold text-gray-800 text-success">
                                                        ...
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input" type="checkbox" role="switch"
                                                                name="firma_digital" value="1" <?= ($curso['firma_digital'] ? 'checked' : '') ?>>
                                ...
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3 d-flex justify-content-between align-items-center">
                                        ...
                                        <button type="button" class="btn btn-success btn-sm"
                                            onclick="agregarNuevoModulo(<?= $curso['id_curso'] ?>)"><i class="fas fa-plus"></i>
                                ...
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3">
                                        <h6 class="m-0 font-weight-bold text-primary">Firmas del Certificado</h6>
                                    </div>
                                    <div class="card-body">
                                        <?php
                                        ...
                                        foreach ($posiciones_firma as $posicion):
                                            ...
                                            <div class="row align-items-center mb-2 border-bottom pb-2">
                                                <div class="col-md-4"><strong><?= h($posicion['descripcion_posicion']) ?></strong>
                                                </div>
                                                <div class="col-md-4">
                                                    <select class="form-select form-select-sm"
                                                        name="config_firmas[<?= $id_pos ?>][id_cargo_firmante]">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox"
                                                            name="config_firmas[<?= $id_pos ?>][usar_promotor_curso]" value="1"
                                                            <?= ($conf && $conf['usar_promotor_curso']) ? 'checked' : '' ?>>
                                ...
                                <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-info" data-bs-toggle="modal"
                                            data-bs-target="#detallesCursoModal<?= $curso['id_curso'] ?>"
                                            title="Estados Tomo y Folios"><i class="fas fa-eye me-1"></i> Estados Tomo y
                                            Folios</button>
                                        <button type="button" class="btn btn-success"
                                            onclick="loadPage('../controllers/buscar.php', { id_curso: <?= $curso['id_curso'] ?> })"><i
                                                class="fas fa-users me-1"></i> Inscripciones</button>
                                        <?php
                                        $estado_texto = $curso['estado'] ? 'Finalizar Curso' : 'Iniciar Curso';
                                        $action_estado = $curso['estado'] ? 'finalizar' : 'iniciar';
                                        $btn_class = $curso['estado'] ? 'btn-danger' : 'btn-warning';
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header bg-info text-white">
                    <h5 class="modal-title"><i class="fas fa-eye me-2"></i>Estados Tomo y Folios</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body p-0">
                    <iframe src="../public/detalles_curso.php?id=<?= $curso['id_curso']; ?>"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" id="inputBusquedaFacilitador" placeholder="Buscar..."
                        onkeyup="buscarFacilitador(event)">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered">
                        ...
                        <tbody id="tablaResultadosFacilitador">
                            <tr>
...
<script>
    ...
    function cambiarEstadoCursoEvaluar(id_curso, action) {
        if (!confirm('¿Estás seguro de que deseas ' + action + ' este curso ahora?')) return;
        $.ajax({
